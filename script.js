...
// Función para manejar los subapartados del acordeón (Cuidados y Características)
document.querySelectorAll('.acordeon-subboton').forEach(subboton => {
    subboton.addEventListener('click', () => {
        const subcontenido = subboton.nextElementSibling;

        // Alterna la clase activo
        subcontenido.classList.toggle('activo');

        // Ajusta la altura máxima automáticamente
        if (subcontenido.classList.contains('activo')) {
            subcontenido.style.maxHeight = subcontenido.scrollHeight + 'px';
        } else {
            subcontenido.style.maxHeight = '0';
        }
    });
});
